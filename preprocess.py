...
for file in ["train", "dev", "test"]:
    ...
    for doc in tqdm.tqdm(data):
        ...
        for i, entity in enumerate(doc["vertexSet"]):
            for mention in entity:
                ...
                mention_list.append((i, len(mention_list), mention["pos"][0], mention["pos"][1], mention["sent_id"]))
        ...
        mentions = [{"id": int(m), "ent_id": int(i), "pos": [int(s), int(e)], "sent_id": int(sen)} for i, m, s, e, sen in mention_list]
    ...
    with open("./data/docred_pre_{}.json".format(file), "w", encoding="utf-8") as f:
