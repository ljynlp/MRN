...
class Vocabulary(object):
    ...
    def __init__(self, rel2id, ner2id, pos2id, frequency=0):
...
def collate_fn(data):
    ...
    ref_mat = torch.zeros((batch_size, max_men, max_men), dtype=torch.long)
    ...
    intra_mat = torch.zeros((batch_size, max_ent, max_ent, RELATION_NUM), dtype=torch.bool)
    for j, x in enumerate(intra_mask):
        intra_mat[j, :x.shape[0], :x.shape[1], :] = x
    intra_mask = intra_mat

    inter_mat = torch.zeros((batch_size, max_ent, max_ent, RELATION_NUM), dtype=torch.bool)
    for j, x in enumerate(inter_mask):
        inter_mat[j, :x.shape[0], :x.shape[1], :] = x
    inter_mask = inter_mat

    return doc_inputs, ner_inputs, dis_inputs, ref_inputs, rel_labels, intra_mask, inter_mask, intrain_mask, \
           doc2ent_mask, doc2men_mask, men2ent_mask, ent2ent_mask, men2men_mask, title


class RelationDataset(Dataset):
    def __init__(self, doc_inputs, ner_inputs, dis_inputs, ref_inputs, rel_labels, intra_mask, inter_mask, intrain_mask,
                 doc2ent_mask, doc2men_mask, men2ent_mask, ent2ent_mask, men2men_mask, title):
    ...
    def __getitem__(self, item):
        return torch.LongTensor(self.doc_inputs[item]), \
               ...
               torch.LongTensor(self.ref_inputs[item]), \
               torch.FloatTensor(self.rel_labels[item]), \
               torch.BoolTensor(self.intra_mask[item]), \
               torch.BoolTensor(self.inter_mask[item]), \
...
def process(data, vocab, is_train):
    ...
    return doc_inputs, ner_inputs, dis_inputs, ref_inputs, rel_labels, intra_mask, inter_mask, intrain_mask, doc2ent_mask, doc2men_mask, men2ent_mask, ent2ent_mask, men2men_mask, title
...
def load_data(load_emb=True):
    ...
    if load_emb:
        with open("./data/DocRED_baseline_metadata/word2id.json", 'r', encoding='utf-8') as f:
